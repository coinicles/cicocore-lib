# Contributing to Cicocore

We're working hard to make *cicocore* the most powerful JavaScript library for working with bitcoin. Our goal is to have *cicocore* be a library that can be used by anyone interested in bitcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in cicocore that any change must adhere.
...
When possible, cicocore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Cicocore methods. For example, calling `JSON.stringify` on an Cicocore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'cicocore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the cicocore object.

DO:

```javascript
var cicocore = require('../');
var PublicKey = cicocore.PublicKey;
```
