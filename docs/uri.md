...
The main use that we expect you'll have for the `URI` class in cicocore is validating and parsing bitcoin URIs. A `URI` instance exposes the address as a cicocore `Address` object and the amount in Satoshis, if present.
